...
The Android app acts as a client while the heavy lifting happens on the backend.

### Frontend
- **Kotlin + Jetpack Compose:** Native Android, modern, less boilerplate, Material Design components.
- Handles APK upload, analysis progress, graphs, reports, and AI chat.

### Backend
- **FastAPI:** Handles `/api/upload`, `/api/analyze`, `/api/report`, `/api/chat`, and `/api/history`.
- **Database:** SQLite for simple starting points, migrating to PostgreSQL for production. Tables include Users, APK Analysis, Reports, Threat History, and AI Chats.

### Processing Pipeline
- **APK Analysis:** JADX (Java, Classes, Resources), Apktool (Manifest, Smali, Resources), Bundletool (AAB, Split APK).
- **Static Analysis (Python):** Uses Androguard, APKInspector, NetworkX, and Scikit-learn to extract Permissions, Activities, Receivers, Services, Intent Filters, Native Libraries, URLs, IPs, and Domains.

### Machine Learning & AI
- **ML Engine:** Starts with a Random Forest model (structural engine) using features like permission counts, API frequency, cyclomatic complexity, reflection usage, dynamic loading, native libraries, entropy, and opcode frequency. Evolving towards Graph Neural Networks.
- **AI Integration:** Uses Gemini API or OpenAI API to explain permissions/APIs, generate mitigation strategies, and summarize threats.

### Visualization & Reports
- **Attack Graph:** Generated by the backend (NetworkX) and rendered on the Android app (GraphView, MPAndroidChart, or Compose Canvas).
- **Reports:** Generated via Python (ReportLab, Markdown, HTML) containing Threat Score, Permissions, Indicators, Behavior, AI Summary, and Recommendations.

### Hosting & Authentication
- **Authentication:** Firebase Authentication / Google Login.
- **Hosting:** Local development (ADB reverse) -> Production (Railway, Render, or DigitalOcean).
...
- **Week 1:** Learn Jetpack Compose, build navigation, APK upload UI.
- **Week 2:** FastAPI backend setup, upload endpoint, store APKs.
- **Week 3:** Integrate JADX and Apktool, parse manifest, extract permissions.
- **Week 4:** Static analysis, risk score computation, AI explanation integration.
- **Week 5:** Attack graph generation, report generation, history screen.
- **Week 6:** Better UI, Authentication, Polishing.
**Result (PAFA v1):** A production-quality APK analysis platform with AI explanations, reports, and visualizations.

### Phase 2: Advanced Analysis (Weeks 7-10)
- Implement Engine B (brand similarity) and Engine C (behavior correlation).
- Refine composite scoring and ML classifiers.
**Result (PAFA v2):** Machine learning integration for structural detection and behavioral correlation.
